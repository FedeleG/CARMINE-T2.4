...
# -*- coding: utf-8 -*-
"""
EURO-CORDEX annual indicators — quick model sensitivity + trend analysis (Spyder script)
...
Outputs:
- Catalog CSV (files found for the selection)
- Summary CSV (trend + sensitivity metrics)
- Plots (PNG):
  - trajectories per scenario (one line per model, plus ensemble median/IQR)
  - spread over time (IQR + min/max)
  - slope distributions per scenario
  - leave-one-out influence on ensemble slope (per scenario)

Notes:
- Trends: simple OLS slope via np.polyfit(year, value, 1). (No p-values unless SciPy is available.)
- Spatial reduction: mean or median over (y,x) to get one value per year per model.
...
# =============================================================================
# FIGURE 1: Trend magnitude by CSA
# - Separate figures per scenario × window × unit-group
# - X-axis: CSA
# - Bars: indicators (top N by |median_slope_decade| within that group)
# - Color: indicator
# =============================================================================

# ---- CONFIG ----
TOP_N_INDICATORS_PER_GROUP = 6
SCENARIO_ORDER = ["rcp26", "rcp45", "rcp85"]
WINDOW_ORDER   = ["near", "near2", "mid", "mid2", "late", "full"]
# Build a label map from your TREND_WINDOWS definition
WINDOW_LABEL = {name: f"{name} ({y0}\u2013{y1})" for name, y0, y1 in TREND_WINDOWS}
TOP_N_DAYS = 6
APPLY_TOPN_ONLY_TO_UNITS = {"days"}
ISOLATE_INDICATORS = {"GDDs"}

FIGDIR = OUTDIR / "fig_trend_byCSA"
...
UNIT_GROUPS = [
    ...
    ("degdays","Temperature (degdays)"),
    ("m/s",    "Wind (m/s)"),
    ("score",  "Index / Score"),
    ("values", "Index / Values"),
]

UNIT_TO_LABEL = {u: lab for u, lab in UNIT_GROUPS}

def unit_label(units: str) -> str:
    u = (units or "").strip()
    return UNIT_TO_LABEL.get(u, f"Other ({u})" if u else "Other")

# ---- LOAD ----
df = pd.read_csv(IN_CSV)
need = {"CSA","indicator","units","scenario","window","median_slope_decade"}
...
# Build global color mapping

# Group indicators by units
indicator_units = (
    df[["indicator", "units"]]
    .drop_duplicates()
    .set_index("indicator")["units"]
    .to_dict()
)

# Define base colormaps per unit
UNIT_COLORMAPS = {
    "days": "Blues",
    "mm": "PuBuGn",
    "degC": "Greens",
...
# Build indicator color dictionary
INDICATOR_COLORS = {}

for unit, cmap_name in UNIT_COLORMAPS.items():
    inds = [i for i, u in indicator_units.items() if u == unit]
    if not inds:
        continue
    ...
    # Avoid very pale colors → use mid-range of colormap
    colors = [cmap(0.4 + 0.5 * (i / max(n-1,1))) for i in range(n)]

    for ind, col in zip(sorted(inds), colors):
        INDICATOR_COLORS[ind] = col

# Order categories (helps stable plots)
df["scenario"] = pd.Categorical(df["scenario"], categories=SCENARIO_ORDER, ordered=True)
df["window"]   = pd.Categorical(df["window"], categories=WINDOW_ORDER, ordered=True)

# CSA order inferred (stable alphabetical)
csa_order = sorted(df["CSA"].unique().tolist())
df["CSA"] = pd.Categorical(df["CSA"], categories=csa_order, ordered=True)

# Add unit label
df["unit_group"] = df["units"].apply(unit_label)

# ---- LOOP: scenario × window × unit_group ----
scenarios = [s for s in SCENARIO_ORDER if s in df["scenario"].astype(str).unique()]
windows   = [w for w in WINDOW_ORDER if w in df["window"].astype(str).unique()]
unit_groups = sorted(df["unit_group"].unique().tolist())

def select_top_indicators(dsub: pd.DataFrame, top_n: int) -> list:
    # Top by absolute median across CSAs (robust ranking for communication)
    rank = (
        dsub.groupby("indicator")["median_slope_decade"]
        .median()
        .abs()
        .sort_values(ascending=False)
    )
    return rank.head(top_n).index.tolist()
...
for sc in scenarios:
    for w in windows:
        ...
        for ug in unit_groups:
            ...
            # Choose top N indicators for this scenario×window×unit-group
            top_inds = select_top_indicators(dug, TOP_N_INDICATORS_PER_GROUP)
            dug = dug[~dug["indicator"].isin(ISOLATE_INDICATORS)].copy()
            if dug.empty:
                continue

            units_here = dug["units"].iloc[0] if not dug.empty else ""
            
            if units_here in APPLY_TOPN_ONLY_TO_UNITS:
                top_inds = select_top_indicators(dug, TOP_N_DAYS)
                dug = dug[dug["indicator"].isin(top_inds)].copy()
                if dug.empty:
                    continue

            # Stable indicator order by abs median slope
            ind_order = (
                dug.groupby("indicator")["median_slope_decade"]
                .median()
                .abs()
                .sort_values(ascending=False)
                .index.tolist()
            ...
            csas = [c for c in csa_order if c in dug["CSA"].astype(str).unique()]
            n_csa = len(csas)
            n_ind = len(ind_order)
            ...
            x = np.arange(n_csa)
            total_width = 0.85
            bar_w = total_width / max(n_ind, 1)
            offsets = (np.arange(n_ind) - (n_ind - 1)/2) * bar_w

            fig_w = max(10, 1.2 * n_csa + 1.2 * n_ind)   # keep readable
            fig, ax = plt.subplots(figsize=(fig_w, 5))

            for i, ind in enumerate(ind_order):
                vals = []
                for csa in csas:
                    sel = dug[(dug["CSA"] == csa) & (dug["indicator"] == ind)]
                    vals.append(float(sel["median_slope_decade"].iloc[0]) if not sel.empty else np.nan)
                ax.bar(
                    x + offsets[i],
                    vals,
                    width=bar_w,
                    label=ind,
                    color=INDICATOR_COLORS.get(ind, "black")
                    )

            ax.axhline(0.0, linewidth=1)
            ax.set_xticks(x)
            ax.set_xticklabels(csas, rotation=0)
            ax.set_ylabel("median slope / decade")
            ax.set_title(f"{ug} — {sc} — {WINDOW_LABEL.get(w, w)}")

            # Legend outside for clarity
            ax.legend(loc="upper left", bbox_to_anchor=(1.01, 1.0), title="Indicator")

            fig.tight_layout()

            # Safe filename
            safe_ug = ug.replace("/", "per").replace(" ", "_").replace("(", "").replace(")", "")
            y_txt = WINDOW_LABEL.get(w, w).replace(" ", "_").replace("–", "-").replace("(", "").replace(")", "")
            outpath = FIGDIR / f"trend_byCSA__{safe_ug}__{sc}__{y_txt}.png"
            fig.savefig(outpath, dpi=220)
            plt.close(fig)
            
iso_dir = FIGDIR / "isolated_indicators"
...
for sc in scenarios:
    for w in windows:
        ...
        for ind in sorted(ISOLATE_INDICATORS):
            d_ind = dsw[dsw["indicator"] == ind].copy()
            if d_ind.empty:
                continue

            csas = [c for c in csa_order if c in d_ind["CSA"].astype(str).unique()]
            x = np.arange(len(csas))
            vals = []
            for csa in csas:
                sel = d_ind[d_ind["CSA"] == csa]
                vals.append(float(sel["median_slope_decade"].iloc[0]) if not sel.empty else np.nan)
            ...
            ax.axhline(0.0, linewidth=1)
            ax.set_xticks(x)
            ax.set_xticklabels(csas)
            ax.set_ylabel("median slope / decade")
            ax.set_title(f"Isolated: {ind} — {sc} — {WINDOW_LABEL.get(w, w)}")
            fig.tight_layout()

            outpath = iso_dir / f"trend_byCSA__isolated_{ind}__{sc}__{w}.png"
            fig.savefig(outpath, dpi=220)
            plt.close(fig)

print(f"[done] Wrote figures to: {FIGDIR.resolve()}")