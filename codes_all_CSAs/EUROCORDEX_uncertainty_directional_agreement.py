...
# -*- coding: utf-8 -*-
"""
EURO-CORDEX annual indicators — quick model sensitivity + agreement analysis (Spyder script)
...
- Directional agreement heatmaps (PNG):
    One heatmap per scenario × time window
    Rows: indicators
    Columns: CSAs
    Color: directional agreement
           (= max(frac_positive, frac_negative), range 0–1)

    Saved to:
    analysis_outputs/fig_agreement_byCSA/
...
# =============================================================================
# STEP 3 (CSA-level view): Agreement heatmap per scenario × window
# - One heatmap per scenario × window
# - Rows: indicator
# - Columns: CSA
# - Color: agreement = max(frac_positive, frac_negative)
# - Window labels include year ranges
# =============================================================================
...
AGREE_DIR = OUTDIR / "fig_agreement_byCSA"
AGREE_DIR.mkdir(parents=True, exist_ok=True)

# Build window label map from your TREND_WINDOWS
WINDOW_LABEL = {name: f"{name} ({y0}–{y1})" for name, y0, y1 in TREND_WINDOWS}

# Load
df = pd.read_csv(IN_CSV)

need = {"CSA","indicator","scenario","window","frac_positive","frac_negative"}
miss = need - set(df.columns)
if miss:
    raise ValueError(f"Missing required columns: {miss}")

# Agreement per CSA row
df["agreement"] = np.maximum(df["frac_positive"].astype(float), df["frac_negative"].astype(float))

# Stable orders
CSA_ORDER = sorted(df["CSA"].unique().tolist())
IND_ORDER = sorted(df["indicator"].unique().tolist())
...
for sc in scenarios:
    for w in windows:
        sub = df[(df["scenario"] == sc) & (df["window"] == w)].copy()
        if sub.empty:
            continue
        ...
        # Reorder
        mat = mat.reindex(index=[i for i in IND_ORDER if i in mat.index])
        mat = mat.reindex(columns=[c for c in CSA_ORDER if c in mat.columns])

        # Optional: sort indicators by mean agreement (descending) for readability
        ...
        ax.set_title(f"Directional agreement (max(frac±)) — {sc} — {WINDOW_LABEL.get(w, w)}")
        ...
        # safe filename with years
        wtxt = WINDOW_LABEL.get(w, w).replace(" ", "_").replace("–", "-").replace("(", "").replace(")", "")
        outpath = AGREE_DIR / f"agreement__indicator_by_CSA__{sc}__{wtxt}.png"
        fig.savefig(outpath, dpi=220)
        plt.close(fig)

print(f"[done] Agreement heatmaps written to: {AGREE_DIR.resolve()}")